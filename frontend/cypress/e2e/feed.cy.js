...
describe('Feed Page', () => {
  ...
  it('should handle modal close', () => {
    ...
    cy.wait(500)
    cy.get('button')
      .contains(/créer un post/i)
      .click()
    cy.get('textarea').should('be.visible')
    cy.get('button[name=close-button]')
        .click()
  ...
  it('should create a post with image upload', () => {
    ...
    cy.get('textarea').should('be.visible')
        .first()
        .type('Test post with image')

    cy.get('input[type="file"]')
        .selectFile('./cypress/dummy.jpg', { force: true })
    cy.get('button')
      .contains(/publier/i)
      .click()
  ...
  it('should comment on a post', () => {
    cy.visit('/feed')
    cy.wait(500)
    const firstPost = cy.get('li.post-content')
        .first()
    firstPost.get('input[name=comment]')
        .first()
        .type('Test comment')
    firstPost.get('button[name=submit-comment]')
        .first()
        .click()
    cy.wait(500)
    firstPost.get('button[name=show-comments]')
        .first()
        .click()
    firstPost.get('.comment-content')
        .first()
        .contains('Test comment').should('exist')
  })
  ...
  it('should show comment count', () => {
    ...
    cy.wait(500)
    cy.get('button')
      .contains(/afficher les commentaires/i)
      .should('exist')
  })
