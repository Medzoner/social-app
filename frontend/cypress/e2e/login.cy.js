describe('Login Page', () => {
  ...
  it('should logout successfully', () => {
    ...
    cy.get('input[type=email], input[name=email], input[name=username]')
      .should('not.be.disabled')
      .type('medz')
    cy.get('input[type=password], input[name=password]').should('not.be.disabled')
      .type('12345')
    cy.get('form')
      .submit()
    cy.url().should('include', '/feed')
    cy.contains(/logout|déconnexion/i)
      .click()
    cy.url().should('include', '/login')
  })
    ...
    cy.get('body')
      ...
      if (
        $body
          .find('button')
          .text()
          .match(/google|oauth/i)
      ) {
        cy.get('button')
          .contains(/google|oauth/i)
          .should('exist')
      }
    })
  })
